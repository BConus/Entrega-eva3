...
const verificar = (id) => {
  ...
  if (input.value.trim() == '') {
        ...
        if (id == 'fecha') {
            ...
            if (dia > 1) {
                input.classList.add('is-invalid')
                div.innerHTML = '<span class="badge bg-danger">No se puede empeñar ayer </span>'
            }
        }
       
        if (id == 'sueldo') {
            if (input.value >= 800000) {
                input.classList.add('is-invalid')
                div.innerHTML = '<span class="badge bg-danger"> no mas de 800.000</span>'
            }
        }
        if (id == 'fono') {
